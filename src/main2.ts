...
onload = () => {
    ...
    function onInputFormula() {
        ...
        const newFormula = formulaInputNode.value.length !== 0
            ? formulaInputNode.value
            : '0';

        if (
            isBrucketsCorrect(newFormula) &&
            isValidFormula(newFormula)
        ) {
            formula = eval(`(x, y, t) => ${newFormula}`);
            t = 0;
        } else {
            console.log(5)
        };
